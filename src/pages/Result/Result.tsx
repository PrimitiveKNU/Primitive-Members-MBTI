...
const resolveDescription = (memberName: string, descriptions: MemberDescription[]) => {
...
function Result() {
  ...
  const descriptions = (descriptionData.description ?? []) as MemberDescription[]
  ...
  const matchedDescription = bestMatch ? resolveDescription(bestMatch.member.name, descriptions) : null
  const matchedImage = bestMatch ? resolveImage(bestMatch.member) : ''
  const descriptionLines = matchedDescription?.desc
    ? matchedDescription.desc.split('\n').filter((line) => line.trim().length > 0)
    : []
    ...
    <section className="result-page">
      <div className="result-shell">
        <div className="result-header">
          ...
          <h1 className="result-title">{matchedDescription?.name ?? bestMatch.member.name}</h1>
        </div>
        ...
        <p className="result-similar">
          {matchedDescription?.name ?? bestMatch.member.name} 와(과) 비슷한 타입인 당신!
        </p>

        <div className="result-actions">
          <button className="result-cta" type="button" onClick={handleReset}>
            다시하기
          </button>
